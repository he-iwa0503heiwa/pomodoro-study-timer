class TimersController < ApplicationController
  ...
  def complete
    # セッションIDを取得
    @study_session = current_user.study_sessions.find(params[:session_id])
    # 現在の時間とスタートの時間から勉強時間を算出
    end_time = Time.current
    duration = (end_time - @study_session.start_time) / 60
    # 勉強時間のセッション
    today_session = current_user.study_sessions.today.completed
    week_session = current_user.study_sessions.this_week.completed
    total_session = current_user.study_sessions.completed
    ...
    if @study_session.update(end_time: end_time, duration: duration, completed: true)
      render json: {
        success: true,
        duration: duration,
        today_total: StudySession.total_duration_for(today_session),
        week_total: StudySession.total_duration_for(week_session),
        all_time_total: StudySession.total_duration_for(total_session)
      }
    else
        render json: { error: @study_session.errors.full_messages.join(", ") }, status: :unprocessable_entity
    end
  ...
  def save_progress
    ...
    if @study_session.update(duration: elapsed_minutes, end_time: Time.current)
      render json: { success: true, minutes_saved: elapsed_minutes, completed: true }
    else
      render json: { error: @study_session.errors.full_messages.join(", ") }, status: :unprocessable_entity
    end
  end
